"""Vetted, deterministic "recipes" for common tasks that are legitimately
codeable but too dangerous to let a small model improvise.

The on-device model classifies "free up disk space" as a coding task correctly
(it is — the deliverable is a cleanup script), but it writes *unsafe* cleanup
scripts (spawning root shells, `rm -rf` of whole cache dirs, wrong paths). So
for disk cleanup the agent does NOT ask the model to write the script — it uses
the hand-written, conservative, report-first script below.

Design rules for FREE_SPACE_SCRIPT:
* Default run is a READ-ONLY report. It deletes nothing without `--apply`.
* It only ever touches regenerable junk: caches, build artifacts, package-
  manager downloads, Trash, Xcode DerivedData. Never documents, never source.
* No `sudo`, ever. It clears the *contents* of cache dirs, not the dirs.
* The "largest items" list is a report only — never auto-deleted.
"""

from __future__ import annotations

FREE_SPACE_SCRIPT = r"""#!/usr/bin/env bash
...
def match_cleanup(task: str) -> bool:
    """True if the task is a disk-space-cleanup request (conservative match)."""
    t = task.lower()
    has_space = any(p in t for p in (
        "disk space", "free up space", "free up some space", "freeing up space",
        "more space", "out of space", "low on space", "storage space",
        "clear cache", "clear caches", "clean up my mac", "free up disk",
    ))
    has_device = any(w in t for w in (
        "mac", "macbook", "computer", "laptop", "device", "drive", "disk", "ssd",
    ))
    return has_space and (has_device or "disk space" in t or "storage" in t)
